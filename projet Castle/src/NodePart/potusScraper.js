            ...
            $(this).find("li").each(function (part2) {
                $(this).find("a").each(function (part3) {
                    if (part3 == 0) {//Premier lien de la dataset de la ligne
                        nomHotel[comptRelaisChateaux] = $(this).filter("a").text().trim();//Récupère la liste des hotels R&C en France
    	...
    	//console.log($(this).find("li").text());
            $(this).find("li").each(function (part2) {
            	...
            	$(this).find(".poi_card-details").each(function (part3) {
            		nomRestaurant[comptMichelin]=$(this).find(".poi_card-display-title").text().trim();//récupère la liste de tous les restos
                    comptMichelin++; 
            	})
        })
            if(i==36)
...
HotelAvecUneEtoile =function(){
	...
	for (var i = 0; i < nomHotel.length; i++) {
        interHotel[i] = JSON.parse(JSON.stringify(nomHotel[i].replace(regex, '')));//Suppression de certains mots clés pour avoir plus d'hotel référencer
        //console.log(interHotel[i]);
            if(nomRestaurant.indexOf(interHotel[i])>=0){
                ...
                //Récupère tous les hotels qui ont un resto étoilé mais pas avec le même nom
                hotelRestoEtoile[comptCompare]=nomHotel[i];
                urlhotelRestoEtoile[comptCompare]=urlHotel[i];
				//console.log(hotelRestoEtoile[comptCompare]);
				comptCompare++;
...
function sleep(ms) {//Fais patienter l'execution pour permettre de charger toutes les pages.
    return new Promise(resolve => setTimeout(resolve, ms));
  }
async function SuppressionRestaurantEtRecupererPrix() {//Suppression des restaurants de la liste et récupération du prix
    await sleep(1000);
    ...
    for (var index = 0; index < hotelRestoEtoile.length; index++) {
        await sleep(500);//Permet de ne pas avoir des pages qui ne sont pas utilisables
        const temp = index;
        const urlHotel = urlhotelRestoEtoile[index];
                        ...
                        $(this).find("a").each(function (partie2) {
                            ...
                            if ($(this).find("span").text() === 'Hôtel') {
                                ...
                                if ($(".price").html() != null) {
                                    ...
                                    urlFinale[comptSuppressionResto]=urlHotel;
...
async function TriInsertion(){//Tri des Hotels par prix
        ...
        urlPhoto.splice(75,1);
        urlFinale.splice(75,1);
        console.log(endFor);
        for(i=1;i<endFor;i++)     // programme tri par insertion
        {
            ...
            prixHotel[j]=test;
            hotelEtoile[j]=inter;
            urlPhoto[j]=urlInterPhoto;
            urlFinale[j]=urlInter;
            if(temp==75)
            {
                ...
                fs.writeFile ("listeHotel.json", test, function(err) {
                    if (err) throw err;
